...
class IndexController extends BaseController{
    ...
    // 分页
    {
        ...
        // 活动
        $db = M('huodong');
        $this->con = $db->where($where)->field('title,id,orders')->order('orders desc')->select();
    ...
    { 
        ...
        $item = M('wx_user')->where($where)->find();
        $this->assign('item', $item);
        if ($_POST) {
            $data     = $_POST;
            $id['id'] = I('get.id');
    ...
    { 
        $db = M('qyinfo');
        $where['inputstatus'] = '1';
        $this->con = $db->where($where)->order('starlet desc')->select();
